...
function EditTapForm (props) {
  ...
  function handleEditTapFormSubmission(event) {
    ...
    props.onEditTap({name: event.target.name.value, brand: event.target.brand.value, price: event.target.price.value, alcoholContent: event.target.alcoholContent.value, quantity: tap.quantity, id: tap.id});
  }
